...
const VideoForm = ({ selectedInterview, favourite,setPaginate, paginate = 1,nextInterview }: { selectedInterview?: any, favourite?: any,setPaginate?:any, paginate?: any,nextInterview?:any }) => {
  ...
  }, [counterSeconds,videoRef?.current?.currentTime]);
  ...
  }, [selectedInterview, videoRef]);

  useEffect(() => {
    if (videoRef?.current) {
      clearLoader();
      if(!isFinite(duration) || isNaN(duration) || duration == undefined ){
  ...
  const clearLoader = () => {
   ...
   const loadingBar = (s = 30) => {
    ...
    const frameDuration = 1000 / animationFrames;
     let seconds:any;
    let okay = (duration != undefined || isFinite(duration) || !isNaN(duration)) ? seconds = duration : seconds="loading"  // Set the desired countdown duration in seconds

    function updateTimer() {
      if (timerElement && progressBar) {
        timerElement.textContent = `${seconds == "loading" ? "loading" : Math.floor(seconds)}` // Format seconds without decimal places
        const progress = ((duration - ((duration != undefined || isFinite(duration) || !isNaN(duration)) ? seconds = duration : seconds="loading")) / duration) * 100;
      ...
      <div className="kjjfds-janwkea4" style={{ overflow: 'hidden', borderRadius: 23, height: 585 }}>
        <div className={`jljdskaflsd d-flex`} style={{ background: 'linear-gradient(182.61deg, rgba(0, 0, 0, 0.2) -0.73%, rgba(0, 0, 0, 0) 93.15%)' }}>
          <div className='kldfjads' style={{ padding: '10px 13px' }}>
            ...
            <div> 
              ...
              <h6>
                <Icons iconNumber={17} />
                {job?.interviewee?.birth_date? getAge(job?.interviewee?.birth_date) : "28"},
        ...
        <div className="kjdflmas-sdmfe kamnask-asnw kljdnas-jdnwd" onClick={() => {
          ...
          {selectedInterview?.videoLink ? (
            <video key={selectedInterview.id} ref={videoRef} style={{ position: 'relative', borderRadius: 23, width: '296px', height: '546px ', top: '0.75%', background: 'black' }} muted={muted}>
              <source src={selectedInterview.videoLink} type="video/mp4" />
              Couldn't load video.
            </video>
          ) : (
            <img src={require("../../images/i6.png")} style={{ position: 'relative', borderRadius: 23,width: '296px', height: '546px ', top: '0.75%' }} />
          )}
          ...
          <div className="ic-muted" onClick={(e) => {
            ...
            {muted ? (
              <svg width="17" height="15" viewBox="0 0 17 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g opacity="0.8">
                  <path d="M9.77576 1.125L5.2861 5.03675H1.06055V9.99164H5.2861L9.77576 13.9034V1.125Z" fill="white" stroke="white" strokeWidth="1.42857" strokeLinecap="round" stroke-linejoin="round" />
                  <path d="M12.4277 5.70703L15.9992 9.27846M15.9992 5.70703L12.4277 9.27846" stroke="white" strokeWidth="1.42857" strokeLinecap="round" />
                </g>
              </svg>
            ) : (
              <svg width="20" height="15" viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g opacity="0.8">
                  <path d="M9.71521 0.707031L5.22556 4.61878H1V9.57367H5.22556L9.71521 13.4854V0.707031Z" fill="white" stroke="white" strokeWidth="1.34558" stroke-linejoin="round" />
                  ...
                  <path d="M12.3398 12.1367C14.4109 12.1367 16.0898 9.89813 16.0898 7.13671C16.0898 4.37529 14.4109 2.13672 12.3398 2.13672" stroke="white" strokeWidth="1.42857" strokeLinecap="round" />
                </g>
              </svg>
          ...
          <div style={{ position: 'absolute', bottom: '95px', left: '135px' }}>
            <svg width="64" height="12" viewBox="0 0 64 12" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="6" cy="6" r="6" fill={paginate == 1 ? '#06FBED' : 'white'} fillOpacity={paginate == 1 ? '0.8' : '0.5'} />
              <circle cx="57.5918" cy="6" r="6" fill={paginate == 2 ? '#06FBED' : 'white'} fillOpacity={paginate == 2 ? '0.8' : '0.5'} />
              <circle cx="31.7969" cy="6" r="6" fill={paginate == 3 ? '#06FBED' : 'white'} fillOpacity={paginate == 3 ? '0.8' : '0.5'} />
            </svg>
          </div>
            ...
            <div className="loading-bar" id="loading-bar">
              ...
              <div style={{ position: 'absolute', right: '12px', top: '15px' }}>
                <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clipRule="evenodd" d="M8.49998 0.980469C12.6423 0.980469 16 4.33825 16 8.48047C16 12.6227 12.6423 15.9805 8.49998 15.9805C4.35791 15.9805 1 12.6227 1 8.48047C1 4.33825 4.35791 0.980469 8.49998 0.980469Z" fill="black" fillOpacity="0.01" stroke="#ADB8FA" strokeWidth="1.25" />
                  <line x1="8.375" y1="8.35547" x2="8.375" y2="4.60547" stroke="#ADB8FA" strokeWidth="1.25" strokeLinecap="round" />
                  <line x1="8.72064" y1="8.58968" x2="11.7303" y2="10.1387" stroke="#ADB8FA" strokeWidth="1.25" strokeLinecap="round" />
                </svg>
              </div>
