...
const VideoForm = ({ selectedInterview, favourite }: { selectedInterview?: any, favourite?: any }) => {
  ...
  const getAllQuestions = (job: any) => {
    ...
    axios.get(process.env.REACT_APP_BACKEND_URL + '/interviews',
      ...
      let filterData = data.filter((item: any)=>{
        if( job.interviewee._id===item.interviewee._id && job.job_id === item.job_id ){
  ...
  useEffect(()=>{},[job, userVideo, showSlide])
    ...
    <div className="kjjfds-janwkea4">
      ...
      <div className="kjdflmas-sdmfe kamnask-asnw kljdnas-jdnwd" onMouseOver={_showHover} onMouseOut={_hideHover}>
        {selectedInterview?.videoLink && selectedInterview?.id && userVideo? (
          <video key={userVideo.id} ref={videoRef} style={{ position: 'relative', borderRadius: 25, width: '99%', background: 'black' }} onTimeUpdate={handleTime} onEnded={handleVideoEnd}>
            <source src={userVideo[showSlide].questions[0].video_url} type="video/mp4" />
            Couldn't load video.
          </video>
        ) : null}
        ...
        <div className='kdjasldk-ajsdmkd'>
          ...
          <div className="slider-dots" >
            <span style={{background:showSlide===0?"#1fdacd":"grey"}} onClick={()=>handleSlider(0)}></span>
            <span style={{background:showSlide===1?"#1fdacd":"grey"}} onClick={()=>handleSlider(1)}></span>
            <span style={{background:showSlide===2?"#1fdacd":"grey"}} onClick={()=>handleSlider(2)}></span>
          </div>
        </div>
        ...
        <div className={`kjdsia-ajdwnkd ${hoverShow ? `d-flex` : `d-${playing ? 'none' : 'flex'}`}`}>
          <Icons iconNumber={25} />
          <h5>{userVideo ? userVideo[showSlide]?.questions[0]?.question_id?.question : 'What are your strengths and weaknesses?'}</h5>
