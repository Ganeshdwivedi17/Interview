...
const JobTitle = ({ setMainScreen, setShowScreen, showMessage, jobData, setJobView }: { setMainScreen: any, setShowScreen: any, showMessage: boolean, jobData: any, setJobView: any }) => {
  ...
  const getClicked = (data: any) =>{
    let id = user?.id;
    if(id == data.interviewer?._id){
      localStorage.setItem("editable", "edit");
    }else{
      localStorage.setItem("editable", "no-edit");
    }
  }
  ...
  return (
    <div className="jobPositionDiv" onClick={() => {
      getClicked(jobData);
      // localStorage.setItem("Questions", JSON.stringify(jobData.questions));
