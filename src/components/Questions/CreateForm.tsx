      ...
      <div className={`kjdfkksd-aweinmd hkasjfdlmf-dsfsd align-items-start ${showScreen == 3 ? "kdjsf-awejdn" : showScreen == 5 ? "kdjsf-awejdn" : ""}`}>
        <div className="absolute top-0 left-0 w-50" style={{marginLeft:26}}>
          ...
          {showScreen == 3 ? (
            <div className="d-flex justify-content-center flex-column">
              <div className="kjdaflj-adjkwmd align-self-center">
                {questions?.map((data: any, index: number,) => (
                  <>
                  <Question key={index}
                    jobViewContext={job?.questions}
                    setselected={setselected}
                    selected={selected}
                    questions={data}
                    questionIds={questionIds}
                    setQuestionIds={setQuestionsIds}
                    mainStyle={data?._id === user?.id ? {
                    ...
                    timerStyle={{
                      ...
                      alignItems: 'center',
                      background: (job?.questions? job?.questions: questionIds).find((qId: any) =>  qId === data._id) ? '' : 'rgba(0, 0, 0, 0.05)'
                    }}
              ...
              <div className="d-flex justify-content-center">
                ...
                {selected >= 3 ? (
                  <button onClick={() => {
                    let {interviewer,questions,job_recruiter,job_title} = job
                   interviewer = user?.id;
                   questions = questionIds;
                    // setJobView(jobFinalData);
                    const FinalDATA:any = {interviewer,questions,job_recruiter,job_title} 
                    setNewJob(FinalDATA);
                    // addJob(jobFinalData)
            ...
            <>
              <CheckForm id={job?._id} showScreen={showScreen} setShowScreen={setShowScreen} questionIds={questionIds} questions={questions} newJob={newJob} setJobView={setJobView} />
            </>
          ) : ( 
            <div className="sfjkdfjsd-dsnaf">

              <div className={`djsfisdmo-sfmef mt-0 
                ...
                `}>
                {
                  showScreen == 1 ? (
                    <>
                      {/* <input id="questionInput" placeholder="e.g Bar Staff Position Available" name='job_title' 
                      className="ojdfkak-ksmd" type="text" onChange={(e) => {
                        handleJobData(e)
                      }} autoComplete="off" value={job?.job_title || ''} autoFocus /> */}

                      <textarea id="questionInput" placeholder="e.g Bar Staff Position Available" name="job_title" 
                      className={`ojdfkak-ksmd ${job?.job_title?.length >= 33 ? "after_textarea" : "before_textarea"}`} onChange={(e) => {
                        handleJobData(e)
                      }} autoComplete="off" value={job?.job_title || ''} autoFocus />
                    ...
                    <>
                      <input id="questionInput" placeholder="e.g Recruitment Agency" name="job_recruiter" className="ojdfkak-ksmd" type="text" onChange={(e) => {
                        handleJobData(e)
                      }} autoComplete="off" value={job == undefined ? "" :job?.job_recruiter || ''} autoFocus />
                    ...
                    <>
                      <textarea id="questionInput" placeholder="e.g What’s your 5 year plan?" name="question" 
                      className={`ojdfkak-ksmd ${newQuestion?.question?.length >= 35 ? "after_textarea" : "before_textarea"}`} onChange={(e) => {
