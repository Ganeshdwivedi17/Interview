...
const SettingMenuIcon = ({ setMainScreen }: { setMainScreen: any }) => {
  ...
  const handleAge = async (id: string) =>{
    try {
      let res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/users/${id}`);
      let birthDate = res.data?.birth_date;
  ...
  useEffect(()=>{
    const userId: string | undefined = user?.chat?.user?.users
      ? Object.keys(user.chat.user.users)[0]
      : undefined;
    ...
    <>
      ...
      {/* } rootClose> */}
        <button  onClick={handleOverlayShow} className="profile-img-btn btn btn-show no-shadow border-0 outline-0">
          {/* <Icons iconNumber={1} /> */}
          <img
            className="profile-img p-0 rounded-circle"
            src={user?.profile_image || profile_img}
            onError={(e: any) => {
              e.target.src = profile_img;
            }}
