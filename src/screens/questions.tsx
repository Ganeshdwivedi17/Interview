...
function View({ mainScreen,fromPreview, setMainScreen, setJobViewContext, setChatUser, jobViewContext, setWatchAns, showSharedNotify, setShowNotify }: { mainScreen: number, setMainScreen: any, setJobViewContext: any, setChatUser: any, jobViewContext: any, setWatchAns: any, showSharedNotify: any,fromPreview?:any, setShowNotify: any }) {
    ...
    <LinearBackground style={{ width: '100%' }}>
      <div className="pageContainer" style={{ padding: 0, gap: 0 }}>
        <div className="leftSideDiv" style={fullscreen ? { width: '100%' } : {}}>
          {
            showScreen <= 5 ?
            <div className="leftsideWhithTransparentBg" style={{ height: 800 }}>
          ...
          <BackMenu setJobViewContext={setJobViewContext} showRightMenu={showRightMenu} setShowRightMenu={setShowRightMenu} showScreen={showScreen} setShowScreen={setShowScreen} />
          {showScreen >= 1 && showScreen <= 5 ? (
            <CreateForm  jobViewContext={jobViewContext}  showScreen={showScreen} setShowScreen={setShowScreen} setJobView={setJobView} />
          ) : (
            <></>
          )}
        ...
        <RightLayout2 setJobViewContext={setJobViewContext} setMainScreen={setMainScreen} setShowScreen={setShowScreen} style={fullscreen ? { display: 'none' } : {}} />
      </div>
    </LinearBackground>
  );
}
